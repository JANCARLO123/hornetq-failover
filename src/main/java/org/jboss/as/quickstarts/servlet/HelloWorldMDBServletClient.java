...
public class HelloWorldMDBServletClient extends HttpServlet {
	...
	private final static Logger LOGGER = Logger.getLogger(HelloWorldMDBServletClient.class.getName().toString());
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		try {
			...
			for (int i = 0; i < MSG_COUNT; i++) {
				message.setText("This is message " + (i + 1) + " from server: " + System.getProperty("jboss.host.name"));
	...
	private ConnectionFactory getConnectionFactory() {
		try {
			...
			LOGGER.log(Level.SEVERE, "Couldn't lookup connection factory...", ex);
		}
		return null;
	}
